...
export async function GET(req: NextRequest) {
  try {
    ...
    // Restrict to agency plan users
    if (user.subscriptionPlan !== 'agency') {
      return new NextResponse('Agency plan required', { status: 403 });
    }
    ...
    // Generate comprehensive analytics data for agency plan
    const analyticsData = {
      overview: {
    ...
    console.error('[AGENCY_ANALYTICS_ERROR]', error);
